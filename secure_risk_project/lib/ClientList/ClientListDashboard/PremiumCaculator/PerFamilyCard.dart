...
class PerFamilyCard extends StatefulWidget {
  // const PerFamilyCard({super.key});
  int? clientId;
  ...
  @override
  State<PerFamilyCard> createState() => _PerFamilyCardState();
}

class _PerFamilyCardState extends State<PerFamilyCard> {
  ...
  Future<void> createFamilyDetails(int? ageBandStart, int? ageBandEnd,
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        await _fetchData();
        Navigator.of(context).pop();
        _showSuccessDialog(context, "Premium Details Added");
        clearTextFeilds();
        ...
        if (data != null && data.isNotEmpty) {
          final List<familyDetails> insurerList = data
              .map((item) => familyDetails(
                    familyid: item['familypremiumCalcuater_id'],
...
Future<void> updatePremiumFamilyDetails(
    int? premiumFamilyid,
    ...
    final Map<String, dynamic> requestBody = {
      ...
      "sumInsured": sumInsured,
      "basePremium": basePremium,
      "familypremiumCalcuater_id": premiumFamilyid,
      "rfqId":"233",
      "productId":"1001",
      "clientListId":"1552"
    };
    ...
    final response = await http.patch(
      Uri.parse(ApiServices.baseUrl +
          ApiServices.updatePremiumFamilyDeatils
          ),
      body: jsonEncode(requestBody),
      headers: headers,
    );
    ...
    if (response.statusCode == 201 || response.statusCode == 200) {
      ...
      _showSuccessDialog(context, "Premium Family Details Updated");
      clearTextFeilds();
    } else {
...
Future<void> updatePremiumFamilyDetailsDialog(int premiumId) {
    List<familyDetails> filteredData = familyList
        .where((premium) => premium.familyid == premiumId)
        .toList();
    ...
    final TextEditingController premiumLifeid = TextEditingController(
        text: filteredData.first.familyid.toString());
 ...
 Future<void> deletePremiumFamilyDetails(String premiumFamilyId) async {
    var headers = await ApiServices.getHeaders();
    try {
      ...
      if (confirmed == true) {
        ...
        final apiUrl = Uri.parse(ApiServices.baseUrl +
            ApiServices.deleteFamilyDetails +
            premiumFamilyId);

        final response = await http.delete(apiUrl, headers: headers);

        if (response.statusCode == 200) {
          print('Premium Family details deleted successfully');
          await _fetchData();
        } else {
          print(
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                      ...
                      columns: [
                        ...
                        buildDataColumn("Age Band Start"),
                        buildDataColumn("Age Band End"),
                        buildDataColumn("Sum Insured"),
                        buildDataColumn("Base Premium"),
                        buildDataColumn("Action")
                      ],
                      rows: [
                        ...familyList
                            .map((insure) => DataRow(cells: [
                                  DataCell(Text((familyList.indexOf(insure) + 1)
                                      .toString())),
                                  DataCell(
                                    ...
                                    children: [
                                      IconButton(
                                          onPressed: () {
                                            ...
                                            updatePremiumFamilyDetailsDialog(insure.familyid);
                                          },
                                          icon: Icon(Icons.edit)),
                                      IconButton(
                                        onPressed: () async {
                                         deletePremiumFamilyDetails(insure.familyid.toString());                                        },
