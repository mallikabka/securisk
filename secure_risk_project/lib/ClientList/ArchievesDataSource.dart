...
import 'package:loginapp/ClientList/ArchieveClientList.dart';
import 'package:loginapp/Colours.dart';
import 'package:syncfusion_flutter_datagrid/datagrid.dart';
import 'package:http/http.dart' as http;
import '../Service.dart';

class ArchievesDataSource extends DataGridSource {
                            ...
                            rows: snapshot.data!
                                .map(
                                  (product) => DataRow(
                                    cells: [
                                      DataCell(Text(
                                          (snapshot.data!.indexOf(product) + 1)
                                              .toString())),
                                      DataCell(Text(product.productName)),
                                      DataCell(Text(product.policyType)),
                                      DataCell(Text(product.tpaId)),
                                      DataCell(Text(product.insurerCompanyId)),
                                      DataCell(
                                        Row(
                                          children: [
  ...
  void _handleDeleteProductList(
      BuildContext context, String productId, void Function() _fetchData) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
    ...
    return DataGridRowAdapter(
      cells: row.getCells().map<Widget>((dataCell) {
        if (dataCell.columnName == 'Actions') {
          ...
          return GestureDetector(
            onTap: () {
              openProductsDialog(context, insurer.clientId, _fetchData,
                  onDesignationChanged, insurer.designation);
            },
            child: Container(
              padding: EdgeInsets.all(5),
