...
class LocationDataSource extends DataGridSource {
  ...
  Future<List<User>> fetchUsers(int id) async {
    ...
    final response = await http.get(
        // Uri.parse(
        //     "http://192.168.5.67:9763/roles/getAllUsersByLocationIdLogin/$id"),

        Uri.parse(ApiServices.baseUrl +
            ApiServices.getAll_usersBy_Location +
            "/" +
            "$id"),
        headers: headers);
  ...
  DataGridRowAdapter? buildRow(DataGridRow row) {
    ...
    return DataGridRowAdapter(
      cells: row.getCells().map<Widget>((dataCell) {
        ...
        } else if (dataCell.columnName == 'user') {
          return GestureDetector(
            onTap: () {
              openUsersDialog(context, insurer.locationId, _fetchData);
            },
            child: Container(
