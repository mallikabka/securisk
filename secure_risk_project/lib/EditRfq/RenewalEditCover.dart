...
import 'package:loginapp/EditRfq/EditEmpDataCard.dart';
...
class RenewalEditCover extends StatefulWidget {
  late final String rfId;

  final GlobalKey<RenewalEditCoverState>? renewalcoverageDetailsKey;
  // final Uint8List fileBytes;
  RenewalEditCover(
      {Key? key, this.renewalcoverageDetailsKey, required this.rfId})
      : super(key: key);

  @override
  State<RenewalEditCover> createState() => RenewalEditCoverState();
...
// Determine which radio button is selected
    ...
    final url = Uri.parse(ApiServices.baseUrl +
        ApiServices.updatecoverageDetails_Endpoint +
        rfId);
...
class RenewalEditCoverState extends State<RenewalEditCover> {
  late RenewalEditEmpData empData;
  ...
  Future<void> fetchFieldMappings() async {
    ...
    empData = RenewalEditEmpData(
        renewalEditCoverageDetailsStateKey: widget.renewalcoverageDetailsKey,
  ...
  void initState() {
    ...
    (widget.renewalcoverageDetailsKey?.currentContext);
    renewalEditClaimsEditData = RenewalEditClaimsMisData(
        renewalEditCoverageDetailsState: widget.renewalcoverageDetailsKey);
    getCoveragesById();
    fetchFieldMappings();
  }
  ...
  Future<void> getCoveragesById() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        OneThree = responseData['familyDefication13Amount'];
        OneFive = responseData["familyDefication15Amount"];
        ParentOnly = responseData["familyDeficationParentsAmount"];
        if (empDataField) {
        ...
        //var isfamilyDefication132 = isfamilyDefication13;
  ...
  bool validateEditCoverageDetails() {
    ...
    if (policyType == null) {
      setState(() {
        policyTypeError = 'Please select policy type';
      });
    } else {
      setState(() {
  ...
  Future<void> uploadEditedEmpdataFile(
      Uint8List? fileBytes, String fileType, String fileName) async {
    try {
      var headers = await ApiServices.getHeaders();
      final uploadApiUrl =
                                        ...
                                        IconButton(
                                          onPressed: () {
                                            if (!isEmpDataUploadSuccessful) {
                                              empData.renewalEdithandleUpload(
                                                  context, "EmpDepData");
                                            }
                                          },
