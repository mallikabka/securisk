...
import 'package:loginapp/Templates/RFQTemplateDataSource.dart';
import 'dart:convert';
...
class RfqTemplatePojo {
  ...
  factory RfqTemplatePojo.fromJson(Map<String, dynamic> json) {
    final actions = json['actions'] as String? ?? 'edit';
    return RfqTemplatePojo(
      id: json['id'] as int,
...
class RFQTemplate extends StatefulWidget {
  RFQTemplate({Key? key}) : super(key: key);

  @override
  State<RFQTemplate> createState() => _RFQTemplateState();
}

List<String> _locations = ['Before', 'After'];

class _RFQTemplateState extends State<RFQTemplate> {
  ...
  String _searchText = '';
  List<RfqTemplatePojo> _filteredData = [];
  String? _selectedFilter;
  List<RfqTemplatePojo> _data = [];
  ...
  Future<void> fetchDataFromApi() async {
    ...
    final response = await http.get(
        Uri.parse(
            ApiServices.baseUrl + ApiServices.getAll_TemplatesData + 'rfq'),
        headers: headers);

    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      setState(() {
        _data = data.map((item) => RfqTemplatePojo.fromJson(item)).toList();
  ...
  List<RfqTemplatePojo> getCurrentPageData() {
  ...
  List<RfqTemplatePojo> getFilteredData() {
  ...
  Widget build(BuildContext context) {
    List<RfqTemplatePojo> currentPageData = getCurrentPageData();
