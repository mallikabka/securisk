...
import 'package:loginapp/Templates/TPATemplateDataSource.dart';
import 'dart:convert';
...
class TPATemplatePojo {
  ...
  factory TPATemplatePojo.fromJson(Map<String, dynamic> json) {
    final actions = json['actions'] as String? ?? 'edit';
    return TPATemplatePojo(
      id: json['id'] as int,
...
class TPATemplate extends StatefulWidget {
   TPATemplate({Key? key}) : super(key: key);

  @override
  State<TPATemplate> createState() => _TPATemplateState();
}

List<String> _locations = ['Before', 'After'];

class _TPATemplateState extends State<TPATemplate> {
  ...
  String _searchText = '';
  List<TPATemplatePojo> _filteredData = [];
  String? _selectedFilter;
  List<TPATemplatePojo> _data = [];
  ...
  Future<void> fetchDataFromApi() async {
    var headers = await ApiServices.getHeaders();
    final response = await http.get(
      Uri.parse(ApiServices.baseUrl + ApiServices.getAll_TemplatesData+'tpa'),headers: headers
    );

    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      setState(() {
        _data = data.map((item) => TPATemplatePojo.fromJson(item)).toList();
  ...
  List<TPATemplatePojo> getCurrentPageData() {
  ...
  List<TPATemplatePojo> getFilteredData() {
  ...
  Widget build(BuildContext context) {
    List<TPATemplatePojo> currentPageData = getCurrentPageData();
    ...
    Future<Map<String, dynamic>> sendFileToBackend(
      ...
      var request = http.MultipartRequest("POST", Uri.parse(apiUrl))..headers.addAll(headers);

      request.fields['templateName'] = templateName.text;
      request.fields['templateType'] = _selectedLocation!;
      request.fields['type'] = 'tpa';
                                    ...
                                    child: ElevatedButton(
                                      ...
                                      child:  Row(
                                        ...
                                        children: [
                                          Icon(
                                            Icons
                                                .add_circle_outline, // You can change this to your desired icon
                                            color: Colors
