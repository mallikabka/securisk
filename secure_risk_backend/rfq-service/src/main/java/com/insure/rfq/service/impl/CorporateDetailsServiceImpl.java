...
public class CorporateDetailsServiceImpl implements CorporateDetailsService {
	...
	public UpdateCorporateDetailsDto updateRFQ(String rfqIdValue, UpdateCorporateDetailsDto corporateDetailsDto) {
		if (rfqIdValue != null && corporateDetailsDto != null) {
            ...
            Optional.ofNullable(corporateDetailsDto.getNobCustom()).ifPresentOrElse(corporateDetailsEntity::setNobCustom,
                    () -> corporateDetailsEntity.setNobCustom(""));
...
//		List<EmployeeDepedentDetailsEntity> findAll = empDepRepo.getByrfqId(id);
//		EmployeeDataReportPdfGenerator pdf = new EmployeeDataReportPdfGenerator();
//		byte[] generateEmployeeDataReport = pdf.generateEmployeeDeptData(findAll);
...
FileInputStream fileInputStream;
	...
	public byte[] getIrdaData(String rfqId) throws IOException {
		CorporateDetailsEntity corporateDetails = corporateDetailsRepo.findByRfqId(rfqId).orElseThrow(); // Assuming this method always returns a value
		...
		CoverageDetailsEntity coverageDetailsEntity = coverageDetailsRepo.findByRfqId(rfqId).orElseThrow(); // Assuming this method always returns a value
		...
		return irda.generateEmployeeDataReport(corporateDetails, expiryPolicyDetails, claimsDetails, coverageDetailsEntity, employeeRelations);
	}
...
//
//        List<CooperateDetailsGraphDto> getAllDashBoardDetails = new ArrayList<>();
//        Map<String, Long> appStatusCounts = new HashMap<>();
//
//        List<CorporateDetailsEntity> corporateDetails = corporateDetailsRepo.findAll().parallelStream()
//                .filter(data -> data.getCreateDate() != null && data.getAppStatus() != null)
//                .filter(data -> {
//                    String[] createDateParts = data.getCreateDate().split(" ")[0].split("-");
//                    int dbYear = Integer.parseInt(createDateParts[0]);
//                    int dbMonth = Integer.parseInt(createDateParts[1]);
//                    return dbYear == year && dbMonth == monthValue;
//                })
//                .collect(Collectors.toList());
//
//        for (CorporateDetailsEntity data : corporateDetails) {
//            String appStatus = data.getAppStatus();
//            appStatusCounts.put(appStatus, appStatusCounts.getOrDefault(appStatus, 0L) + 1);
//        }
//
//        long totalData = appStatusCounts.values().stream().mapToLong(Long::longValue).sum();
//
//        for (Map.Entry<String, Long> entry : appStatusCounts.entrySet()) {
//            getAllDashBoardDetails.add(CooperateDetailsGraphDto.builder()
...
//                .getCooperateDetailsGraphDtos(getAllDashBoardDetails)
