...
public class ClientListFamilyPremiumCalcuaterServiceimpl implements ClientListFamilyPremiumCalcuaterService {
    ...
    public List<ClientListFamilyPremiumCalcuaterDto> getAllTheClientListPremiumCalcuaters(Long clientID, Long productId) {
        ...
        return calcuaterRepository.findAll()
                .stream().filter(status -> status.getRecordStatus().equalsIgnoreCase("ACTIVE") && status.getClientListId().getCid() == clientID && status.getProductId().getProductId().equals(productId)
                )
                .map(i -> {
...
public ClientListFamilyPremiumCalcuaterDto getClientListPremiumCalcuaterDto(Long primary_Id) {
        List<ClientList_PreFamilyPremium_Calcuater> active = calcuaterRepository.findAll().stream()
                .filter(i -> i.getRecordStatus().equalsIgnoreCase("ACTIVE") &&
                        i.getPremiumCalcuater_id().equals(primary_Id)).collect(Collectors.toList());
    ...
    public byte[] generateExcelFromData(Long clientListId, Long productId) {
        ...
        List<ClientList_PreFamilyPremium_Calcuater> familyPremiumCalcuaters = calcuaterRepository.findAll().
        ...
        List<ClientList> list = familyPremiumCalcuaters.stream().map(ClientList_PreFamilyPremium_Calcuater::getClientListId).toList();
