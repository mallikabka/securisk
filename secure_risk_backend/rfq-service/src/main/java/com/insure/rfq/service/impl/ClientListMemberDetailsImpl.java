...
//        }
	...
	public List<GetAllClientListMembersDetailsDto> getAllClientListMembersDetails(Long clientListId, Long productId) {
		...
		List<GetAllClientListMembersDetailsDto> listOfMembersDetails = clientListMemberDetailsRepository.findAll()
				...
				.filter(c -> productId != null && c.getProduct().getProductId().equals(productId)).map(c -> {
					...
					try {
						...
						// getAllClientListMembersDetailsDto.setDate(dateTime.format(DateTimeFormatter.ofPattern("dd",
						// Locale.ENGLISH)));
					} catch (NullPointerException | DateTimeParseException e) {
						getAllClientListMembersDetailsDto.setMonth(null);
						// getAllClientListMembersDetailsDto.setDate(null);
					}
...
//   @Scheduled(cron = "0 0/1 * 1/1 * ?")
	...
	public List<GetClientListMemberDetailsActiveListCountDto> getAllActiveListForMemberDetailsByClientListProduct(
		...
		// Get current date up to 12:00 midnight
		...
		return activeMembers.stream().map(c -> {
			...
			long employeeCount = activeMembers.stream()
					.filter(member -> member.getRelationShip().equalsIgnoreCase("Employee")).count();
			long dependentCount = activeMembers.stream()
					.filter(member -> !member.getRelationShip().equalsIgnoreCase("Employee")).count();
			long totalCount = employeeCount + dependentCount;
			...
			try {
				...
				cal.setTime(new SimpleDateFormat("yyyy-MM-dd").parse(c.getDateOfBirth()));
...
//    }
	...
	private boolean isCellMergedOrEmpty(Cell cell) {
		...
		if (cell.getSheet().getNumMergedRegions() > 0) {
			for (int i = 0; i < cell.getSheet().getNumMergedRegions(); i++) {
				CellRangeAddress mergedRegion = cell.getSheet().getMergedRegion(i);
	...
	public ClientListEnrollmentHeaderDto validateHeadersBasedOnTpa(MultipartFile multipartFile, String tpaName)
		...
		ClientListEnrollmentHeaderDto haderDto = new ClientListEnrollmentHeaderDto();
...
//        log.info("list of headers :{}", validHeaders);
		validHeaders.forEach(i -> {
			if (headerList.contains(i.getHeaderName()) || headerList.contains(i.getHeaderAliasName())) {
				switch (i.getHeaderName()) {
				case "Employee Id": {
					haderDto.setEmployeeIdStatus(true);
					break;
				}
				case "Employee Name": {
					haderDto.setEmployeeNameStatus(true);
					break;
				}
				case "Date of Birth": {
					haderDto.setDateOfBirthStatus(true);
					break;
				}
				case "Gender": {
					haderDto.setGenderStatus(true);
					break;
				}
				case "Age": {
					haderDto.setAgeStatus(true);
					break;
				}

				case "Relation": {
					haderDto.setRelationStatus(true);
					break;
				}
				case "Date of Joining": {
					haderDto.setDateOfJoiningStatus(true);
					break;
				}
				case "E Card Number": {
					haderDto.setECardNumber(true);
					break;
				}
				case "Policy Start Date": {
					haderDto.setPolicyStartDateStatus(true);
					break;
				}
				case "Policy End Date": {
					haderDto.setPolicyEndDateStatus(true);
					break;
				}
				case "Base Sum Insured": {
					haderDto.setSumInsuredStatus(true);
					break;
				}
				case "Group Name": {
					haderDto.setGroupNameStatus(true);
					break;
				}
	...
	@Override
	public List<ClientListMemberDetailsDataStatus> validateValuesBasedOnTpa(MultipartFile multipartFile,
			String tpaName) {
	...
	public void validateBasedOnSheet(Sheet sheet, String tpaName,
		...
		List<ClientListEnrollementHeadersMappingEntity> validHeaders = clientListEnrollementHeadersMappingRepository
				.findByTpaName(byTpaName.getTpaName());
//		log.info("Valid Headers :{}", validHeaders);
		...
		if (!validHeaders.isEmpty()) {
			...
			for (Cell cell : headerRow) {
				if (isMergedCell(sheet, cell)) {
					...
					columnIndex++;
					continue; // Skip processing merged cells
				}
				...
				if (columnName.isEmpty()) {
					...
					columnIndex++;
					continue; // Skip processing empty headers
				}
				...
				for (ClientListEnrollementHeadersMappingEntity header : validHeaders) {
					...
					if (columnName.equalsIgnoreCase(headerName) || columnName.equalsIgnoreCase(headerAliasName)) {
						...
						foundHeaders = true; // Set to true if any header is found
					}
				}
				columnIndex++;
			...
			if (dataRow.getCell(employeeIdColumnIndex) != null) {
				...
				if (employeeIdCell.getCellType() == CellType.NUMERIC) {
					...
					while (prevIndex >= 0) {
						Cell prevCell = dataRow.getCell(prevIndex);
						if (prevCell != null && prevCell.getCellType() == CellType.STRING) {
							String employeeId = prevCell.getStringCellValue().trim();
							if (!employeeId.isEmpty()) {
								validateDto.setEmployeeId(employeeId);
								validateDto.setEmployeeIdStatus(true);
								break; // Exit the loop if a non-empty employee ID is found
...
//
	...
	@Override
	public List<GetAllClientListEnrollmentDto> getAllclientListEnrollmentData(Long productId, Long clientListId) {
