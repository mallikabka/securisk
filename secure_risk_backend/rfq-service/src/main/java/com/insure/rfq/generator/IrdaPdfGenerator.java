...
public class IrdaPdfGenerator {
	...
	public byte[] generateEmployeeDataReport() throws IOException {
		...
		try {
			...
			// I/We am/are authorized to propose on behalf of these persons."));
...
//                          : ", "+corporateDeails.getTpaContactName())))
			...
			if (expiry != null) {
				...
				additionPremium = expiry.getAdditionPremium();
				deletionPremium = expiry.getDeletionPremium();
			...
			long employeeSum = relation.stream()
					.filter(str -> filterValues.stream().anyMatch(value -> str.equalsIgnoreCase(value))).count();
