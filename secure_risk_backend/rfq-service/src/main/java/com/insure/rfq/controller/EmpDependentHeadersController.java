...
public class EmpDependentHeadersController {
	...
	private ResponseEntity<EmpDependentHeaderDto> addHeader(@RequestBody EmpDependentHeaderDto header){
		...
		if(header != null) {
			...
			return new ResponseEntity<EmpDependentHeaderDto>(createHeaders, HttpStatus.CREATED);
		}
		return new ResponseEntity<EmpDependentHeaderDto>(createHeaders, HttpStatus.BAD_REQUEST);
	}
